...
function removerIntegrantesPorID() {
  // la classname de los input cambia cuando devuelven un error, no puedo usar solamente borrarIntegrantesAnteriores()
...
function demostrarEdades() {
  ...
  document.querySelector(
    "#edad-mayor"
  ).innerText = `La edad mayor es ${obtenerNumeroMayor(numeros)}`;
  document.querySelector(
    "#edad-menor"
  ).innerText = `La edad menor es ${obtenerNumeroMenor(numeros)}`;
  document.querySelector(
    "#edad-promedio"
  ).innerText = `El promedio de las edades es ${calcularNumeroPromedio(
    numeros
  )}`;
}
...
document.querySelector("#calcular").onclick = function () {
  ...
  for (i = 0; i < contador; i++) {
    edadIntegrante = document.querySelector(
      "[name=edadNombre" + String(i) + "]"
    ).value;
